...
export default async function handler(req: any, res: any) {
    ...
    try {
        const upstream = await fetch('https://biwenger.as.com/api/v2/user?fields=players(*,clause,owner)', {
            headers: {
                'Content-Type': 'application/json',
                'Accept': 'application/json',
                'Authorization': req.headers.authorization || '',
                'X-League': req.headers['x-league'] || '',
                'X-User': req.headers['x-user'] || '',
            },
        });

        return await forwardJson(res, upstream, 'Biwenger user');
    } catch (error) {
